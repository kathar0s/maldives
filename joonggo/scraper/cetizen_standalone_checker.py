...
class CetizenStandaloneChecker(DjangoSpider):
    ...
    def start_requests(self):
        print(u'CetizenStandaloneChecker start')

        index = 0
        articles = Article.objects.filter(Q(source=self.ref_object) & Q(survival_count__gte=1)).order_by('id')[:10]
        print(u"checking (%d) articles\n" % (len(articles)))
        for article in articles:
            print(u"checking : %d %s\n" % (article.id, article.url))
            yield Request(article.url, callback=self.detail_parse, meta={'article': article})
            time.sleep(1)

        print(u'CetizenStandaloneChecker ends')
